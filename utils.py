...
def get_test_data(test_ratio=0.99):
    ...
    shuffled_data_set = [data_set[(i + seed) % len(data_set)] for i in range(len(data_set))]
    test_separation_point = int(test_ratio * len(data_set))
    test_set, training_set = shuffled_data_set[:test_separation_point], shuffled_data_set[test_separation_point:]
